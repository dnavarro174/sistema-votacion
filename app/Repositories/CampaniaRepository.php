...
class CampaniaRepository implements ICampaniaRepository
{
    ...
    //function send($id, $default=[])
        ...
        */
        ...
        $xurl = "https://www.enc-ticketing.org/au/enc/";
        ...
        try {
            echo "Pasa por SendHistoryEmails $flujo - line: 91\r\n";
            if($tipo == "EMAIL"){
                ...
                if($flujo=="MAILING"){
                    echo "MAILING - SendHistoryEmails line: 110 \r\n";
                    $plantilla_id = $historyEmail["plantilla_id"];
                    $plantilla = Plantillaemail::find($plantilla_id);
                    $html = $plantilla->plantillahtml;
                    echo resource_path().'/views/email/'.$plantilla_id.".blade.php \r\n";

                    $file=fopen(resource_path().'/views/email/'.$plantilla_id.'.blade.php','w') or die ("error creando fichero!");
                    fwrite($file,$html);
                    fclose($file);
                    $mail_send = Mail::send("email.{$plantilla_id}", $data, function ($mensaje) use ($email, $nombres, $asunto, $from_email, $from_nombre ){
                ...
                if($flujo=="NEWSLETTER"){
                    $mail_send = Mail::send("email.envio_newsletter", $data, function ($mensaje) use ($email, $nombres, $asunto, $from_email, $from_nombre ){
                        $mensaje->from($from_email, $from_nombre);
                        $mensaje->to($email, $nombres)->subject($asunto);
                    });
                ...
                if($flujo=="INVITACION"){
                    ...
                    if($usuarios_count>0){
                        ...
                        $mail_send = Mail::send("email.{$msg_text}", $data, function ($mensaje) use ($email, $nombres, $asunto, $from_email, $from_nombre ){
                            $mensaje->from($from_email, $from_nombre);
                            $mensaje->to($email, $nombres)->subject($asunto);
                        });
                        $result=RESULT_HOY.RESULT_MENSAJE;
                ...
                if($flujo == "CONFIRMACION" OR $flujo == "RECORDATORIO"){
                    ...
                    if($nrs_gafete > 0){
                        // si tiene gafete
                        $mail_send = Mail::send("email.{$msg_text}", $data, function ($mensaje) use ($email, $nombres, $asunto, $file, $from_email, $from_nombre ){
                            $mensaje->from($from_email, $from_nombre);
                            $mensaje->to($email, $nombres)->subject($asunto);
                            $mensaje->attach($file);
                        });
            ...
            if($tipo == "WHATS"){
                ...
                if($paq_msn->mensajes >= $paq_msn->cant){
                    ...
                    $mail_send = Mail::send('email.notificacion', $data, function ($mensaje) use ($data, $from_nombre, $from_email){
                        $mensaje->from($from_email, $from_nombre);
        ...
        $status = strstr($result,RESULT_MENSAJE)&& ! $mail_send instanceof \Illuminate\Mail\SentMessage ? 4 : $error;
        
        if($is_test)return true;
        #Log::info(json_encode($historyEmailData)." -  $result --  $status");
