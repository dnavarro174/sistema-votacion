...
class ConfirmacionController extends Controller
{
    ...
    public function index(Request $request, EstudianteRepository $repository, CampaniaRepository $campaniaRepository)
    {
        ...
        if($export==1){
            $datos = $this->filtros($data);
            return $this->exportaXLS($datos);

        }


        $data['pag'] = $pag;
        if($search != ''||$tipo>0||$modalidad>0||$grupo!=''||$apro!=''||$region>0){
    ...
    public function chocolatear($datos, $evento_id, $cn=0, CampaniaRepository $campaniaRepository)
    {
        ...
        $rs_datos1 = $this->getPlantillaXEvento($evento_id, 1);
        ...
        foreach($datos as $estudiante){
            ...
            $rs_datos = ($existePlantVirtual2 == 0) ? $rs_datos1: $rs_datos2;
            ...
            $confirm_email = ($SI) ?$rs_datos->p_conf_inscripcion_email: $rs_datos->p_baja_evento_email;
            $confirm_msg   = ($SI) ?$rs_datos->p_conf_inscripcion_msg: $rs_datos->p_baja_evento_msg;
            ...
            if($modalidad==1){
                ...
                $msg_cel = ($SI) ?$rs_datos->p_conf_inscripcion_2: $rs_datos->p_baja_evento_2;
            }else{
                // VIRTUAL
                ...
                $msg_cel = ($SI) ? $rs_datos->p_conf_inscripcion_2_v : $rs_datos->p_baja_evento_2_v;
            }
            ...
            if(!$SI){
                ...
                ## ENVIAR INVITACION A RECHAZADOS, PASADO A MOD. VIRTUAL

                $flujo_ejecucion = ($SI) ? 'INVITACION': 'INVITACION';

                $plant_confirmacion = ($SI) ? $rs_datos->p_conf_inscripcion : $rs_datos->p_conf_inscripcion_v;
                $plant_confirmacion_2 = ($SI) ? $rs_datos->p_conf_inscripcion_2 : $rs_datos->p_conf_inscripcion_2;

                $confirm_email = ($SI) ?$rs_datos->p_conf_inscripcion_email: $rs_datos->p_conf_inscripcion_email;
                $confirm_msg   = ($SI) ?$rs_datos->p_conf_inscripcion_msg: $rs_datos->p_baja_evento_msg;

                $modalidad=$modalidad==1?2:1;

                if($modalidad==1){
                    $asunto =  ($SI) ? $rs_datos->p_conf_inscripcion_asunto: $rs_datos->p_conf_inscripcion_asunto;
                    $msg_text = ($SI) ?$rs_datos->p_conf_inscripcion: $rs_datos->p_conf_inscripcion;// plantila email
                    $msg_cel = ($SI) ?$rs_datos->p_conf_inscripcion_2: $rs_datos->p_conf_inscripcion_2;
                }else{
                    // VIRTUAL
                    $asunto = ($SI) ? $rs_datos->p_conf_inscripcion_asunto_v : $rs_datos->p_conf_inscripcion_asunto_v;
                    $msg_text = ($SI) ? $rs_datos->p_conf_inscripcion_v : $rs_datos->p_conf_inscripcion_v;// plantila email
                    $msg_cel = ($SI) ? $rs_datos->p_conf_inscripcion_2_v : $rs_datos->p_conf_inscripcion_2_v;
                }
            ...
            if($confirm_email == 1){
                ...
                if($email != ""){
                    $xcelular = '';
                    $xmsg_cel = '';
                    $this->insertaHistoriaEmail('EMAIL',$dni, $id_plantilla, $flujo_ejecucion, $asunto, $nom, $email, $xcelular, $msg_text, $xmsg_cel, $from_name, $from_email,$mod_desde);
            ...
            if($confirm_msg == 1){
                ...
                if($celular != "" && strlen($estudiante->celular)>= 9){
                    if(!$SI)$msg_text = '';
                    $this->insertaHistoriaEmail('WHATS',$dni, $id_plantilla, $flujo_ejecucion, $asunto, $nom, $email, $celular, $msg_text, $msg_cel, $from_name, $from_email,$mod_desde);
                }
            }
    ...
    public function eliminarVarios(Request $request){
        ...
        if(!is_null($request->selection_si)){
            ...
            foreach ($seleccion_si as $value) {
                ...
                $nom = $estudiante->nombres .' '.$estudiante->ap_paterno;
                $email = $estudiante->email;
                $rs_datos = $this->getPlantillaXEvento(session('eventos_id'),1);
                ...
                $this->actualizaTrackXEstudiante($value);
                    ...
                    if($rs_datos->p_conf_inscripcion_email == 1){
                        ...
                        if($email != ""){
                            #$celular = '';
                            $this->insertaHistoriaEmail('EMAIL',$dni, $id_plantilla, $flujo_ejecucion, $asunto, $nom, $email, '', $msg_text, $msg_cel, $from_name, $from_email,$mod_desde);
                        }
                    }else{
        ...
        if(!is_null($request->selection_no)){
            foreach ($seleccion_no as $value) {
                ...
                if($rs_datos){
                    ...
                    $confirm_email = $rs_datos->p_baja_evento_email;
                    $confirm_msg   = $rs_datos->p_baja_evento_msg;
                    ...
                    if($confirm_email == 1){
                        if($email != ""){#$msg_cel = '';#$celular = '';
                            $this->insertaHistoriaEmail('EMAIL',$dni, $id_plantilla, $flujo_ejecucion, $asunto, $nom, $email, '', $msg_text, '', $from_name, $from_email,$mod_desde);
                        }
                    }else{
                        // no inserta en la tb historia_email
                        $error .= 'No se envio el email porque no esta habilitado en la configuración del EVENTO.<br>';
                    ...
                    // MSG WHATS
                    if($confirm_msg == 1){
                        if($celular != "" && strlen($estudiante->celular)>= 9){$email = '';$msg_text = '';
                            $this->insertaHistoriaEmail('WHATS',$dni, $id_plantilla, $flujo_ejecucion, $asunto, $nom, $email, $celular, $msg_text, $msg_cel, $from_name, $from_email,$mod_desde);
                        }
                    }else{
                        $error .= 'No se envio el whatsapp porque no esta habilitado en la configuración del EVENTO.<br>';
    ...
    private function filtros($data=[])
    {
        ...
        $st = Estudiante::join('estudiantes_act_detalle as de','estudiantes.dni_doc','=','de.estudiantes_id')
            ...
            ->when($random, function($query) {
                ...
                $query->orderBy('de.id', request('sorted', 'DESC'));
            })
            ->select('estudiantes.id','de.id as codigo','estudiantes.dni_doc','estudiantes.nombres','estudiantes.ap_paterno','estudiantes.ap_materno',
                ...
                'de.daccedio as accedio','estudiantes.created_at','de.dtrack','de.modalidad_id','de.estudiantes_tipo_id');
    ...
    private function actualizaTrackXEstDet($dni, $evento_id, $dtrack='SI'){
        if($dtrack=='SI'){
            ...
            ## CAMBIAR MOD. PRESENCIAL A VIRTUAL TODOS LOS RECHAZADOSS
            return DB::table('estudiantes_act_detalle')
                ->where('eventos_id', $evento_id)
                ->where('estudiantes_id', $dni)
                ->update([
    ...
    private function insertaHistoriaEmail($tipo,$dni, $id_plantilla, $flujo_ejecucion, $asunto, $nom, $email, $celular, $msg_text, $msg_cel, $from_name, $from_email,$mod_desde="")
    {
        if(!$tipo)$tipo = 'EMAIL';
        $data = [
            'tipo'              => $tipo,
    ...
    function exportaXLS($datos)
    {
        $name = 'reporte-'.date('d-m-y-H-i-s').'.xls';
        ini_set('max_execution_time', 300000);
        ini_set('memory_limit', '4096M');
        $headers = [
            [
                '#', '', 'DNI', 'Apellidos_y_Nombres', 'Grupo', 'Modalidad', 'Tipo', 'Email', 'Cargo',
                'Entidad', 'Profesión', 'Celular', 'País', 'Departamento', 'Registrado', 'FechaReg'
            ]
        ];
        $colWidths = [
            'A' =>  8,
        ...
        if($datos->count()>0){
            foreach($datos as $datos){
                $data[] = array(
                    ...
                    $datos->dgrupo,
                    optional($datos->Modalidad)->modalidad,
                    optional($datos->tipo)->nombre,
                    $datos->email,
                    $datos->cargo,
                    $datos->organizacion,
                    $datos->profesion,
        ...
        $data = compact( "headers", "styles", "data", "colWidths", "colFormats");
        return \Excel::download(new GeneralExport($data), $name);
    }
