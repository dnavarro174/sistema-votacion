...
class InvImportacionesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view("invimp.index", compact('camps', 'permisos', 'default_nombre'));
...
// Outside a loop...
    ...
    public function EstudianteImportSave(Request $request, EstudianteRepository $repository)
    {
        ...
        #$filename0 = "storage/excel/imports/". $invimp->id . '.'. $extension;
        $filename0 = "storage/excel/imports/". $filename;
        $filename2 = public_path($filename);

        $file_path  = "storage/excel/". $name;
        $file_old = public_path($file_path);
        
        $file_path2  = "storage/excel/imports/". $invimp->id . '.'. $extension;
        $file_new = public_path($file_path2);

        $invimp->filesize = \File::size($file_old);#;
        \File::move($file_old, $file_new);
        $invimp->file = $file_path2;
        #if(\File::exists($file_new))\File::delete($file_new);
        $invimp->save();
        #dd($filename,$filename0,$filename2,$file_old,$file_new);
