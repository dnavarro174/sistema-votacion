...
use App\Models\LabsMobileService;
//use Auth;
...
class EnvioEmailController extends Controller
{
    	...
    	$prin = "";$xid = ""; $error = "";
	    	if(count($datos_email) > 0){
		    	...
		    	foreach ($datos_email as $key => $value) {
					...
					$body = 'Mensaje 2 desde Laravel';
					$send = VonageService::send($phone_number, $body);
					dd($send);*/
		    			...
		    			$msg_text 	= $value->msg_text;//plantila email p_preregistro_2
						    ...
						    $datos_email = array(
				                ...
				                if($flujo_ejecucion == "CONFIRMACION" OR $flujo_ejecucion == "RECORDATORIO"){
									...
									if($id_lista == 269 or $id_lista == 344 or $id_lista == 356 or $id_lista == 357 ){
				                    ...
				                    $data = array(
										...
										#NUEVOS CAMPOS PARA PLANTILLA-HTML
										'dni'   	=> $dni,
										'eventos_id'=> $id_lista
				                    ...
				                    if($eventos->gafete==1){
											...
											if($flujo_ejecucion=="RECORDATORIO"){
												// PRESENCIAL

												if($modalidad==1){
													$asunto = $eventos->p_recordatorio_asunto;
													$msg_text = $eventos->p_recordatorio;// plantila email
													$msg_cel = $eventos->p_recordatorio_2;//msg what
													$pantallazo = $eventos->p_recordatorio_gracias;
												}else{
													// VIRTUAL
													$asunto = $eventos->p_recordatorio_asunto_v;
													$msg_text = $eventos->p_recordatorio_v;// plantila email
													$msg_cel = $eventos->p_recordatorio_2_v;// msg what
													$pantallazo = $eventos->p_recordatorio_gracias_v;
												}
											}
											//Eventos diferentes a CAII
											if($modalidad==0){
												$msg_text 	= $value->msg_text;//plantila email p_preregistro_2
												$msg_text = ($eventos_id==357)?'357p_conf_registro':$msg_text;
												#DD($dni,$eventos_id ,$datos_email['from_email'],$msg_text,$datos_email,$data);
    ...
    private function sendTo(...$usuarios) {
        ...
        foreach ( $usuarios as $usuario ) {
			...
			#$body = "Mensaje de Usuario y Contraseña para el evento CAII 2023.";
			#dd($telefono, $body);
			$body = str_replace("\n"," ", $body);
            if($usuario['pdf_url']!=""){
				...
				$result[] = LabsMobileService::send($telefono, $body, 'CONTRALORIA');
