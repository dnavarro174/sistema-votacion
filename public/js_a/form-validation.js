...
function have_excluded(){
  return $('#exclude').length ;
}
function getExcluded(){
  return have_excluded() &&  $('#exclude').prop('checked');
  //return excluded = !!($excluded.length && $excluded.prop('checked'));
}
...
(function($) {
  ...
  $(function() {
    ...
    $("#btnSumImport").click( function(){
      console.log('btnSumImport... paso');
      $("#tipoImport").val('curso');
      $("#estudiantesImportSave, #cursosImportSave").submit();
      }
    );

    /* $("#btnSumImportCGR").click( function(){
      console.log('btnSumImportCGR click');
      $("#tipoImport").val('cgr');
      $("#estudiantesImportSaveCGR").submit();
      }
    ); */
    ...
    $('#btnCerrar, .btnCerrar_curso').click((e) => {
      location.reload();
    });
    ...
    $("#auto_conf").on('change', function(){
      ...
      console.log('click auto_conf',tipo);

      if(tipo == "1"){
        $("#auto_conf_div, .campos_opcles").removeClass('d-none');
        $('#opc_1, #email_id, #p_conf_registro, #email_asunto').attr({'required':true});
        $('#confirm_email').attr({"checked":true});
        $('.hidden_email').removeClass('hidden_email').addClass('quit_hidden_email');
      }else{
        $("#auto_conf_div, .campos_opcles").addClass('d-none');
        $('#opc_1, #email_id, #p_conf_registro, #email_asunto').attr({'required':false});
        $('#confirm_email').attr({"checked":false});
        $('#email_asunto,#email_id,#fecha_texto').val('');
        $('.quit_hidden_email').addClass('hidden_email');
      }
    });
    ...
    $dni_doc.bind("change",function(e){
      ...
      $.get(url, function(resp,depa){
        console.log('probando...',resp.datos.length);
        ...
        if(resp.datos.caso==2){
          console.log('caso 2'); 

          $('#nombres').val(resp.datos.datos[0].nombres);
          $('#nombres').val(resp.datos.datos[0].nombres);
          $('#ap_paterno').val(resp.datos.datos[0].ap_paterno);
          $('#ap_materno').val(resp.datos.datos[0].ap_materno);
          $('#profesion').val(resp.datos.datos[0].profesion);
          $('#organizacion').val(resp.datos.datos[0].organizacion);
          $('#cargo').val(resp.datos.datos[0].cargo);
          $('#xemail').val(resp.datos.datos[0].email);
          $('#xcelular').val(resp.datos.datos[0].celular);

          // proveedor de dominio
          /*
          let cadena_email = resp.datos.datos[0].email;
          let email_usuario = "";
          let email_dominio = "";
          ...
          email.val(resp.xdatos.email).attr('disabled',true);//resp.xdatos.email
    ...
    $nom_curso.bind("change",function(e){
      ...
      $.get(url, function(resp,depa){
        ...
        if(resp.datos.length > 0){
          ...
          let vcapa = Number(resp.datos[0].valor_capa);
          $('.valor_capacitacion').val(vcapa.toFixed(2));
    ...
    $('#Recordatorio').click(function(e) {
      ...
      $("#Recordatorio").text('Procesando...');

      $.get(url,function(resp, resul){
        $("#Recordatorio").text('Recordatorio');
    ...
    $("#estudiantesForm").validate({ 
      ...
      messages: {
        ...
        inputdni: {
          ...
          maxlength: "DNI debe máximo 18 digitos",
        },
    ...
    // Mensajes IMPORTAR
    function mensaje_validacion(_flag,_flagDni,_tipo){
      console.log(_flag,_flagDni,_tipo);
      if(_flag==0){
        swal({ type:'info',title:'Advertencia', text:'Por favor asignar al menos una columna', icon:'warning', showConfirmButton: false,timer: 2000});
      ...
      if(_flagDni==0){
          swal({ type:'info',title:'Advertencia', text:'El '+_tipo+' es un campo obligatorio', icon:'warning', showConfirmButton: false,timer: 2000});
          $("#btnSumImport").removeAttr("disabled");
          exit();
          return false;
      }
    ...
    function importar_success(_tipo){
      swal({
      ...
      if(_tipo=="Codigo"){
        console.log('pasoo tipo:',_tipo);
        $("#btnSumImportCGR").removeAttr("disabled");
        $("#iframePrevCGR").attr("style","display: block;");
        $("#estudiantesImportSaveCGR").attr("style","display: none");
        $("#btnSumImportCGR").attr("style","display: none");
        $("#iframePrevCGR").attr("style","display: none");
        //sleep(500);
        document.getElementById("iframePrevCGR").contentDocument.location.reload(true);
        
        $("#iframePrevCGR").attr("style","display: block;border: 1px solid #e6e6e6;");
      }else{
        $("#btnSumImport").removeAttr("disabled");
    ...
    //------------------------ESTUDIANTES-------------------------//
    //estudiantesImportSaveCGR
    
    $('form#estudiantesImportSave, form#cursosImportSave').submit( function( e ) {
        e.preventDefault();
        console.log('estudiantesImportSave.... form ');
        ...
        if(getExcluded())_flagDni = 1;
        mensaje_validacion(_flag, _flagDni, _tipo);
        console.log("tipo:",_tipo);
        $("#cargador_excel2").attr("style","display: block; ");
      ...
      $("#btnRegresar").click(function(){
        $("#btnSumImport").attr("disabled",false);
        _flagMod = 1;
        eximForm();
      })
     ...
     $("#archivo").change(function (){
        _flagMod = 0;
        console.log('Form seleccionar campos');
        $("#f_cargar_datos_estudiantes").submit();
        $("#f_cargar_cursos").submit();
     });
    ...
    // form cargar datos excel
    $('form#f_cargar_datos_estudiantes').submit( function( event ) {
      console.log('lista estudiantes caii...');
         ...
         // $("#btnImport1").removeAttr("disabled");
          ...
          $.ajax({
              ...
              success: function(datos){
                ...
                _combo = _combo + "<option value=21>MODALIDAD</option>";
                if(tipo_evento==2){
                ...
                */
                ...
                if(datos.length>0){
                  ...
                  console.log('datos n',datos[0].length);
    ...
    // form cargar datos excel cursos
    $('form#f_cargar_cursos').submit( function( event ) {
      console.log('leyendo excel cursos');
      $("#btnImport1").attr("disabled","disabled");
      $("#cargador_excel").attr("style","display:block");
      event.preventDefault();        
    }).validate({
      ...
      submitHandler: function(form) {
        ...
        $.ajax({
            ...
            success: function(datos){
              let tipo_dj = $('#tipo_dj').val();
              let tipo_form = $('#tipo_form').val();
              console.log('tipo_dj...',tipo_dj);
              console.log('tipo_form...',tipo_form);
              $("#hdnTabla").val(datos);
              $("#btnImport1").removeAttr("disabled");    
              $("#cargador_excel").attr("style","display:none");       
              ...
              $('#Modal_estudiantes').modal('hide');
              $('#tbl_estudiantes_imp_ord').html("");
              let _combo = "<select class='form-control border-info text-uppercase' required onchange='cmbOrganiza(this)' id='cambiar' name='cambiar' style='width:200px'>";
              _combo = _combo + "<option value=0></option>";
              if(tipo_form=="cgr"){
                _combo = _combo + "<option value=1>CÓDIGO</option>";
                _combo = _combo + "<option value=5>DNI</option>";
                _combo = _combo + "<option value=3>APELLIDO.PATERNO</option>";
                _combo = _combo + "<option value=4>APELLIDO.MATERNO</option>";
                _combo = _combo + "<option value=2>NOMBRES</option>";
                _combo = _combo + "<option value=6>CATEGORIA</option>";
                _combo = _combo + "<option value=7>UNIDAD_ORGANICA</option>";
                _combo = _combo + "<option value=8>EMAIL</option>";

              }else{
                _combo = _combo + "<option value=1>CÓDIGO</option>";
                _combo = _combo + "<option value=2>NOMBRE_CURSO</option>";
                _combo = _combo + "<option value=3>MODALIDAD</option>";
                _combo = _combo + "<option value=4>FECHA INICIO</option>";
                _combo = _combo + "<option value=5>FECHA FIN</option>";
                
                if(tipo_dj==10){
                  _combo = _combo + "<option value=6>TIPO CAPACITACION</option>";
                  _combo = _combo + "<option value=7>PROVEEDOR DE CAPACITACION</option>";
                  _combo = _combo + "<option value=8>HORAS DE CAPACITACION</option>";
                  _combo = _combo + "<option value=9>COSTOS DIRECTOS</option>";
                  _combo = _combo + "<option value=10>COSTOS INDIRECTOS</option>";
                  _combo = _combo + "<option value=11>VALOR DE LA CAPACITACION</option>";
                  _combo = _combo + "<option value=12>MATERIA DE CAPACITACION</option>";
                }
              }
