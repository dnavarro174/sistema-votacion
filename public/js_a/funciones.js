function buscarusuario(){
  ...
  function Confirmacion(a){
      var $tipo = $("#"+a);
      var $capaMostrar = $("."+a+"_1");
      $tipo = $tipo.val();
      console.log($tipo);

      if($tipo == "1"){
        $capaMostrar.removeClass('d-none');
        /* $('#email_id_pre, #p_conf_registro_pre, #email_asunto_pre').attr({'required':true});
        $('#confirm_email_pre').attr({"checked":true}); */
      }else{
        $capaMostrar.addClass('d-none');
        /* $('#email_id_pre, #p_conf_registro_pre, #email_asunto_pre').attr({'required':false});
        $('#confirm_email_pre').attr({"checked":false});
        $('#email_asunto_pre,#email_id_pre').val(''); */
      }
  }

  function checkMostrar(input,tipo,clase){
    //function checkMostrar(a,b,c,d=''){
    let $check = $('#'+input+"_"+tipo);
    let $clase = $('.'+clase);

    //let hijo = $('#'+clase+" input");
    ...
    //if ($(this).is(':checked')) {
      ...
      $($clase).each(function(){
        ...
        $(this).find('input').attr({'required':false}).val('');
        $(this).find('textarea').attr({'required':false}).val('');
        
        console.log('each efectuado no');
      });
      
     }

  //prereg_confirm_email p_preregistro _email
  //prereg_confirm_msg p_preregistro_2 _msg
...
function formActividad(evento_id,dia, actividad_id, num , url){ 
  ...
  function openTerminos(){$("#Modal_estudiantes").modal('show');}
