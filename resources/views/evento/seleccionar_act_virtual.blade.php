                    ...
                    <div class="col-sm-12 col-md-12  grid-margin stretch-card">
                      <div class="card">
                        <img src="{{ asset('images/form')}}/{{$datos->img_cabecera}}" alt="{{$datos->nombre_evento}} {{date('Y')}}" class="img-fluid">
                          ...
                          <div class="card-body">
                            <h4 class="card-title text-danger2">Indicaciones - Modalidad: Virtual</h4>{{-- ELECCIÓN DE ACTIVIDADES --}}
                            @if(Session::has('actividades_selec'))
                              <p class="alert alert-danger">{!! Session::get('actividades_selec') !!}</p>
                            @endif

                            <p class="card-text">
                              Gracias por validar sus datos.<br>
                              Agradeceremos seguir las siguientes indicaciones:
                            </p>
                            <ul>
                              <li>Las actividades que elijan serán registradas en sistema y podrá verlas en su gafete y correo electrónico de confirmación.</li>
                              ...
                              <li>Debe asistir puntualmente portando su DNI y su gafete virtual.</li>

                            </ul>
                            <p class="card-text">
                              A continuación, deberá seleccionar la actividad en la que desea participar:
                            </p>
                                    ...
                                    <div class="row">
                                      <div class="col-sm-12 col-md-12 sinpadding_mobile">
                                        ...
                                        @foreach($d["horas"] as $j=>$d2)
                                          @if($loop->first)
                                          <h2 class="card-title tit_act text-danger2">
                                            <span>{{$dia_texto}} </span>
                                            <em class="linea" ></em>
                                          </h2>
                                          @endif
                                          ...
                                          <h4 class="card-title color_gris text-transform-none"><span class="resaltado">{{$d2["hora_inicio"]}} HRS.</span> - Elegir una actividad en este bloque</h4>

                                          <div class="col-sm-12 col-md-12 pb-3  ">
                                            @foreach($d2["actividades"] as $k=>$d3)
                                            <div class="row  @if($d3["vacantes_v"] <= $d3["inscritos_v"]) alert alert-danger @endif ">
                                              <div class="col-sm-12 col-md-12 form-control2">
                                                <label class="form-radio">
                                                  <input type="radio" id="varios[{{$i}}_{{$j}}]" class="form-check-" name="varios[{{$i}}_{{$j}}]" value="{{$d3["id"]}}" required="" @if($d3["vacantes_v"] <= $d3["inscritos_v"]) disabled @endif>
                                              ...
                                              <div class="col-sm-12 col-md-9 text-justify">
                                                ...
                                                <p class="mb-0"><strong>Capacidad: </strong>{{$d3["vacantes_v"]}} </p>
                                                <p class=""><strong>Ubicación:</strong> {{$d3["ubicacion"]}}</p>

                                                {!!$d3["desc_ponentes"] !!}
                                                <p>{!!$d3["desc_actividad"] !!}</p>

                                                <?php if($d3["vacantes_v"] <= $d3["inscritos_v"]) echo "<p class='text-center pt-3'><strong>Las vacantes para esta actividad se encuentran agotadas.</strong></p>" ?>
