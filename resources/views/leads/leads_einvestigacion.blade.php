          ...
          <div class="card">
            <div class="card-body">
              <div class="d-flex justify-content-between align-items-center">
                <h4 class="card-title">SUBDIRECCIÓN DE ESTUDIOS E INVESTIGACIONES
                  <a class="btn btn-link" href="{{ route('grupo-estudio-investigacion.index') }}"><i class="mdi text-link mdi-keyboard-backspace"></i> Volver</a>
                </h4>
                  ...
                  <div class="row">{{-- cap: opciones --}}
                    ...
                    <div class="col-xs-12  col-sm-8 text-left mb-4">
                      @if(@isset($permisos['nuevo']['permiso']) and  $permisos['nuevo']['permiso'] == 1)
                        <a href="{{ route('forme_link.create', array('id'=>session('eventos_id')))}}" title="Agregar" class="btn btn-dark btn-sm icon-btn " target="_blank"><i class="mdi mdi-plus text-white icon-md" ></i></a>
                      @endif
                      ...
                      <div class="btn-group" role="group">
                          ...
                          <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                            <a class="dropdown-item" href="{{route('reportes.e_registrados')}}?t=5">Registrados</a>
                          </div>
                      </div>
                      @endif
                        ...
                        <table id="order-listing" class="table table-hover table-sm">
                          <thead class="thead-dark">
                            <tr role="row">
                              ...
                              {{-- <th style="width: 2%;">#</th> --}}
                              <th class="text-center" style="width: 5%;">Passport</th>
                              <th style="width: 3%;">Pronom</th>
                              ...
                              <td><input type="checkbox" class="form btn-delete" name="tipo_doc[]" value="{{ $datos->id }}" data-id="{{ $datos->id }}"></td>
                                <td>
                                  @if(@isset($permisos['editar']['permiso']) and  $permisos['editar']['permiso'] == 1)
                                    <a href="{{route('forme_link.show',['id'=>$datos->id_datos,'eventos_id'=>session('eventos_id'),'tipo'=>$_GET['tipo']])}}">{{ $datos->dni_doc }}</a>
                                  @else
                                    {{ $datos->dni_doc }}
                                  @endif
                                </td>
                                <td>{{ $datos->p_pronom }}</td>
                                <td>{{ $datos->nombres.', '.$datos->ap_paterno }}</td>
                                ...
                                <td class="text-center">@if($datos->p_passport_photo!="")<a href="file/p_passport_photo/{{$datos->detalle_id}}/5">Ver</a>@endif</td>
                                <td class="text-center">@if($datos->investigation!="")<a href="file/investigation/{{$datos->detalle_id}}/5">Ver</a>@endif</td>
                                {{-- <td>
                                  <span class="badge @if($datos->estudiantes_tipo_id === 1)badge-primary @elseif($datos->estudiantes_tipo_id === 2)badge-success @elseif($datos->estudiantes_tipo_id === 3)badge-danger @else($datos->estudiantes_tipo_id === 4)badge-dark @endif ">{{ $datos->tipo->nombre or '' }}</span>
                                </td> --}}
