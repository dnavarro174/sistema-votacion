            ...
            <div class="card-body">
              <div class="d-flex justify-content-between align-items-center">
                <h4 class="card-title">Leads / Registros
                  ...
                  @else
                  <a class="btn btn-link" href="{{ route('grupo-maestria.index') }}"><i class="mdi text-link mdi-keyboard-backspace"></i> Volver</a>
                  @endif
                </h4>
                    ...
                    <div class="form-row">
                      ...
                      <div class=" col-sm-2 col-xs-12">
                        <select class="form-control" name="g" id="g" onchange="submit();">
                          <option selected="selected" value="">APTOS</option>
                      ...
                      <div class=" col-sm-2 col-xs-12">
                        <select class="form-control" name="reg" id="filter-by-date" onchange="submit();">
                          <option selected="selected" value="">APROBADOS EXAMÉN</option>
                          <option value="1">SI</option>
                          <option value="0">NO</option>
                        </select>
                      </div>
                    ...
                    <div class="col-xs-12  col-sm-8 text-left mb-4">
                      ...
                      <div class="btn-group" role="group">
                          ...
                          <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                            <a class="dropdown-item" href="{{route('reportes.e_registrados')}}?t=4">Registrados</a>
                          </div>
                      </div>
                      @endif
                      @if(@isset($permisos['nuevo']['permiso']) and  $permisos['nuevo']['permiso'] == 1)
                        ...
                        <table id="order-listing" class="table table-hover table-sm">
                          <thead class="thead-dark">
                            <tr role="row">
                              ...
                              <th style="width: 3%;">TipoDoc</th>
                              {{-- <th style="width: 2%;">#</th> --}}
                              <th class="text-center" style="width: 8%;">DNI</th>
                              <th style="width: 25%;">Apellidos y Nombres</th>
                              <th style="width: 10%;">GradoProf</th>
                              
                              <th style="width: 5%;">Email</th>
                              <th style="width: 5%;">Celular</th>
                              <th style="width: 5%;">Telefóno</th>
                              <th style="width: 5%;">CGR</th>
                              <th style="width: 10%;">C.P</th>
                              <th style="width: 10%;">D.J</th>
                              <th style="width: 10%;">F.I</th>
                              <th style="width: 5%;">C.V</th>
                              <th style="width: 5%;">Foto</th>
                              <th style="width: 5%;" title="Historial">H</th>
                              <th style="width: 10%;" class="text-center">Voucher</th>
                              <th style="width: 10%;" class="text-center">Fecha Depósito</th>
                              <th style="width: 10%;" class="text-center">{{ ($_GET['tipo']==4)?'Período':'Grupo' }}</th>
                              <th style="width: 10%;">País</th>
                              <th style="width: 10%;">Departamento</th>
                              {{-- <th style="width: 5%;">Tipo</th> --}}
                              {{-- <th style="width: 5%;">Apto Postular</th>
                              <th style="width: 5%;">Aprobó Examén</th> --}}
                          ...
                          <tbody>
                            ...
                            <tr role="row" class="odd" <?php if($datos->dtrack == "SI") echo "style='background:#a0e8c5;'"?> <?php if($datos->dtrack == "NO") echo "style='background:#f7d3d3;'"?>>
                              ...
                              {{-- <td>{{ $datos->updated_at->format('d.m.Y H:i') }}</td> --}}
                              <td>{{ $datos->created_at->format('d/m/Y') }} <br>{{ $datos->created_at->format('H:i:s') }}</td>
                                <td nowrap="">
                                  {{ $datos->tipodoc?\Illuminate\Support\Str::limit($datos->tipodoc->tipo_doc,5):'' }}
                                </td>
                                <td>
                                ...
                                <td>{{ $datos->si_cgr==1?'SI':'NO' }}</td>
                                <td>@if($datos->compago!="")<a href="file/compago/{{$datos->detalle_id}}/4">Ver</a>@endif</td>
                                <td>@if($datos->decjur!="")<a href="file/decjur/{{$datos->detalle_id}}/4">Ver</a>@endif</td>
                                <td>@if($datos->ficins!="")<a href="file/ficins/{{$datos->detalle_id}}/4">Ver</a>@endif</td>
                                <td>@if($datos->cv!="")<a href="file/cv/{{$datos->detalle_id}}/4">Ver</a>@endif</td>
                                <td>@if($datos->cv!="")<a href="file/foto/{{$datos->detalle_id}}/4">Ver</a>@endif</td>
                                <td>@if($datos->link_detalle!="")<a target="_blank" title="Ver Historial" href="{{$datos->link_detalle}}">Ver</a>@endif</td>
                                <td class="text-center">{{ \Illuminate\Support\Str::limit($datos->nvoucher, 25) }}</td>
                                <td class="text-center">{{ date('d-m-Y', strtotime($datos->fecha_conf)) }} </td>
                                <td class="text-center">{{ $datos->dgrupo or '' }} </td>
                                <td> {{ $datos->pais, '' }} </td>
                                <td> {{ $datos->region, '' }} </td>
                                {{-- <td>
                                  <span class="badge @if($datos->estudiantes_tipo_id === 1)badge-primary @elseif($datos->estudiantes_tipo_id === 2)badge-success @elseif($datos->estudiantes_tipo_id === 3)badge-danger @else($datos->estudiantes_tipo_id === 4)badge-dark @endif ">{{ $datos->tipo->nombre,'' }}</span>
                                </td> --}}
