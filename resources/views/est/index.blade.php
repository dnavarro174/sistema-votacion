    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
        ...
        <div class="content-wrapper p-0 mt-3">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Estudiantes <a class="btn btn-link" href="{{ route('academico.index') }}"><i class="mdi mdi-reply-all"></i> A. Académicas</a></h4>
              {{-- <p>{{session('academico_id')}}</p> --}}
                  ...
                  <form>
                    <div class="form-row">
                      <div class=" col-sm-10 col-xs-12">
                        ...
                        <?php if (isset($_GET['s'])){ ?>
                            <a class="ml-2 small btn-cerrar h4" title="Borrar busqueda" href=' {{route('est.index')}} '><i class='mdi mdi-close text-lg-left'></i></a>
                        <?php } ?>
              ...
              <div id="capaEstudiantes" class="row">
                <div class="col-12">
                  ...
                  {{ Form::open(array('route' => array('est.eliminarVarios'), 'method' => 'POST', 'role' => 'form', 'id' => 'form-delete','style'=>'display:inline')) }}
                  ...
                  <div class="row">{{-- cap: opciones --}}
                    ...
                    <div class="col-xs-12  col-sm-8 text-left mb-4">
                      @if(@isset($permisos['nuevo']['permiso']) and  $permisos['nuevo']['permiso'] == 1)
                        <a href="{{ route('est.create') }}" title="Agregar" class="btn btn-dark btn-sm icon-btn ">
                      ...
                      <a href="#" @if($evento_vencido != 1) id="Recordatorio" @endif class="btn btn-sm @if($evento_vencido != 1) btn-dark @else btn-secondary @endif" data-id='{{session('eventos_id')}}' data-mod='eventos'><i class="mdi mdi-email icon-md"></i>Recordatorio</a>
                      @endif
                          ...
                          <tbody>
                            ...
                            <tr role="row" class="odd" <?php if($datos->dtrack == "SI") echo "style='background:#a0e8c5;'"?> <?php if($datos->dtrack == "NO") echo "style='background:#f7d3d3;'"?>>
                              ...
                              <td nowrap="">
                                  ...
                                  <a href="{{ route('est.show',$datos->id)}}" class="">
                                ...
                                <td>
                                  @if(@isset($permisos['editar']['permiso']) and  $permisos['editar']['permiso'] == 1)
                                    <a href="{{route('est.edit', $datos->id)}}" class="btn btn-link p-0"> 
                                    {{ $datos->dni_doc }}
                                    </a>
                                  @else
                                    {{ $datos->dni_doc }}
...
<div class="modal fade ass" id="Modal_estudiantes" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form  id="f_cargar_datos_estudiantes" name="f_cargar_datos_estudiantes" method="post"  action="{{ route('est.import') }}" class="formarchivo" enctype="multipart/form-data" >
          {!! csrf_field() !!}
      ...
      <div class="modal-body pt-0">
          {{-- <div class="form-group row">
            ...
            <div class="col-md-9">
              <a href="{{ route('est.export') }}" class="btn btn-secondary btn-block">Exportar</a>
          ...
          @else
          <p>Evento finalizado. {{$evento_vencido}}</p>
          @endif
    ...
    <div class="modal-content" style="max-height: 600px;">
      ...
      <div class="card">
        <div class="card-body" style=" overflow: scroll;max-height:520px;">
          <iframe src="{{ route('est.importresults') }}" frameborder="1" width="100%" height="400" id="iframePrev" style="display:none; border: 1px solid #e6e6e6;"></iframe>

          <form class="form-inline"  id="estudiantesImportSave" name="estudiantesImportSave" action="{{ route('est.importsave') }}" method="post" >
            {!! csrf_field() !!}
          <div class="row">
