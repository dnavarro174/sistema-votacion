    ...
    <div id="formBuscar" @if($save==1) class=" d-none" @endif>
        <div class="form-group row">
            ...
            <div class="col-sm-5 pl-0">
                <button type="button" title="Buscar" class="btn btn-sm btn-success btn-sm icon-btn " id="curso-buscar" data-search="{{ url('') }}/agregar_colaboradores">
                    Buscar <i class="mdi mdi-arrow-right-drop-circle text-white icon-md" ></i>
                </button>{{-- importarCursos --}}
                <button class="btn btn-sm btn-dark text-white btnImport" onclick="openModal('personalcgr')" id="importarColaboradores-2" type="button"
                    data-toggle="modalImportar" data-target="#modalRemote" data-remote="{{ url('') }}/colaboradores_import"
                    data-backdrop="static" data-title="Importar Cursos" data-fc="form-codigo">
                    <i class="mdi mdi-settings text-white icon-md"></i> ImportarCGR
                </button>
                {{-- <button class="btn btn-sm btn-dark text-white btnImport" onclick="openModal('curso')" id="importarCursos-2" type="button"
                    data-toggle="modalImportar" data-target="#modalRemote" data-remote="{{route('importar_cursos')}}"
                    data-backdrop="static" data-title="Importar Cursos" data-fc="form-codigo">
                    <i class="mdi mdi-settings text-white icon-md"></i> Importar8
                </button> --}}
        ...
        <div class="table-responsive fixed-height" style="height: 500px; padding-bottom: 49px;">{{-- table-responsive-lg --}}
            ...
            <table id="order-listing" class="table table-hover table-sm">
                ...
                <tr role="row">
                    ...
                    <th class="sorting" style="width: 8%;">CODIGO.</th>
                    <th class="sorting" style="width: 5%;">NOMBRES</th>
                    <th class="sorting" style="width: 15%;">PATERNO</th>
                    <th class="sorting" style="width: 15%;">MATERNO</th>
                    <th class="sorting" style="width: 5%;">DNI</th>
                    <th class="sorting" style="width: 20%;">CATEGORIA</th>
                    {{-- <th class="sorting" style="width: 20%;">CARGO</th>
                    <th class="sorting" style="width: 20%;">COND.LABORAL</th>
                    <th class="sorting" style="width: 5%;">COD.AREA</th> --}}
                    <th class="sorting" style="width: 20%;">UNIDAD.ORGANICA</th>
                ...
                @foreach ($dat as $datos)
                    <tr role="row" class="odd hover" style='background:{{ $loop->index%2==0?'f7d3d3':'#e4dfdf' }};' >
                        ...
                        <td style="display: block;white-space: initial !important;width:100px;">
                            {{ $datos->codigo }}
                            {{-- <a href="#" class="curso-edit" tag="{{$datos->id}}"></a> --}}
                        </td>
                        <td>{{ $datos->nombres }}</td>
                        ...
                        <td>{{ $datos->unidad_organica }}</td>
                        <td>{{ $datos->email }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
            {!! $dat->appends(request()->query())->links() !!}
        </div>
    </div>
</div>
