        ...
        <div class="content-wrapper pt-0 bg-white" style="width: 100%;border:none;">
          ...
          <form id="f_enviarInvitacionE" method="post" action='{{ route('estudiantes.enviarI') }}'>
            ...
            <div class="card w-100 bg-white" style="overflow-x:scroll;">
              <div class="card-body">
                <h4 class="card-title">Resultado de Importaci&oacute;n <span class="text-right"><a class="h6" href="{{ route('estudiantes.importresults') }}" target="_blank">Ver informe</a></span></h4>

                <div class="row">
                  <div class="col-12">
                    @if($dat)
                      <span class="badge badge-info">Nuevo: {{$dat->exitoso}}</span>
                      <span class="badge badge-success">Actualizados: {{$dat->actualizado}}</span>
                      <span class="badge badge-danger">No validos: {{$dat->no_valido}}</span>
                      <span class="badge badge-warning">Repetidos: {{$dat->repetidos}}</span>
                      <span class="badge badge-dark">Total: {{$dat->total}}</span>
                    @endif
                    
                      <table class="table dataTable no-footer" role="grid" aria-describedby="order-listing_info" border="0">
                        <thead>
                            <tr role="row">
                              <th>{{-- <strong>IMPORTADOS: </strong> --}}</th> 
